...
Under the unified-conversation wire:
- First event is a **handshake** (`{snapshot_uuid, source_id_assignments, ...}`),
  not a `partition_uuid` event.
- `bot_message` marks final commit with the server-assigned bot `source_id`.
- `compaction_pending` is the last persistence-relevant event when present.
"""
...
async def test_handshake_arrives_first(web_harness, authed_client):
    """The first NDJSON event is a handshake carrying `snapshot_uuid` and
    `source_id_assignments`. There is no top-level `partition_uuid` event."""
...
async def test_bot_message_marks_final_commit(web_harness, authed_client):
    """A successful turn emits exactly one `bot_message` event with a fresh
    server-assigned `source_id`."""
...
async def test_tool_round_order_matches_provider(web_harness, authed_client, request):
    """Provider-specific tool-round ordering. Anthropic emits context_pct
    before progress_message+tool_call; OpenAI emits tool_call first."""
    conversation_uuid = str(uuid4())
    ...
    # First event is the handshake; isolate the remaining event vocabulary.
    assert _is_handshake(events[0])
    types_after = _event_types(events[1:])
    if provider == "anthropic":
        # Anthropic ordering: context_pct → progress_message → tool_call → context_pct → text_delta → bot_message
        assert types_after[:3] == ["context_pct", "progress_message", "tool_call"]
    else:
        # OpenAI ordering: tool_call → context_pct → progress_message → context_pct → text_delta → bot_message

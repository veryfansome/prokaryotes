...
Migrated to the unified-conversation wire (handshake first event, bot_message
final commit, snapshot_uuid + source_id_assignments).
"""
...
async def test_tool_call_round_trip(web_harness, authed_client, request):
    ...
    # TurnExecution should carry both function_call and function_call_output.
    te = await web_harness.search_client.get_turn_execution(record.bot_message_source_id)
...
async def test_think_tool_round_trip(web_harness, authed_client):
    ...
    # The think tool implementation calls llm_client.complete which returns think_text.

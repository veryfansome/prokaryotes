...
Helpers wrap the new handshake/bot_message protocol so per-test code stays terse:
- POST /chat → TurnRecord (snapshot_uuid + source_id_assignments + bot_source_id + text + events).
- `apply_assignments` stamps server-assigned source_ids back onto the local message list.
- `echo_assistant` appends the assistant entry with its captured source_id so the next
  POST passes the DAG-scoped guardrail.
"""
...
def echo_assistant(messages: list[dict], record: TurnRecord) -> None:
    """Append the assistant entry with its server-assigned source_id so the
    next POST's DAG-scoped guardrail recognizes it."""

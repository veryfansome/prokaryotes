...
Migrated to the unified-conversation wire vocabulary:
- First stream event is a `handshake` carrying `snapshot_uuid` (was `partition_uuid`).
- `bot_message` marks final commit with the server-assigned bot `source_id`.
  Subsequent POSTs must echo the assistant entry with that `source_id`.
- `/compaction-status` takes `pending_snapshot_uuid` (was `pending_partition_uuid`).
- Redis cache parses to `Conversation` (was `ContextPartition`).
"""
...
class TurnRecord:
    ...
    `snapshot_uuid` comes from the handshake. `bot_message_source_id` is set
    when the stream emits a `bot_message` (None if the turn aborted before
    commit — e.g., max-rounds hit, stream error). `assistant_text` is the
    accumulated `text_delta` content.
    """
...
async def wait_for_compaction(
    ...
    *,
    ...
    """Poll /compaction-status until done. Returns the relabel target snapshot_uuid
    (or None if no relabel target was written — e.g., lock released without commit)."""
...
def _apply_assignments(messages: list[dict], assignments: list[dict]) -> None:
    """Stamp server-assigned source_ids onto the local message list using the
    handshake's client_index → source_id map."""

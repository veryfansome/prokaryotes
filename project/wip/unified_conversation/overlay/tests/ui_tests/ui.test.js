 ...
 *   (legacy `partition_uuid` event is gone).
 * - `bot_message` marks the final commit with the bot's server-assigned `source_id`.
 * - Other events (`context_pct`, `text_delta`, `progress_message`, `tool_call`,
 *   `compaction_pending`) keep their shape.
 *
 * The DOM/fetch flow inside `createChatApp` is exercised manually in the
 * browser; the pure primitives are covered by `conversation_client.test.js`.
 */
...
describe("parseStreamPayloadLine — legacy partition_uuid is gone", () => {
    it("a payload carrying ONLY partition_uuid is treated as unknown", () => {
        // The old wire's first event shape. Should NOT be recognized as a
        // handshake (which requires source_id_assignments alongside snapshot_uuid).

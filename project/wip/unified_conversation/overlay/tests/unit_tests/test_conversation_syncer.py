...
These run the full pipeline (load → split → resync → assign → reconcile → apply
→ persist) against the in-memory fakes. They cover Issues 1, 3, and the
ordering invariants between the compacted-prefix split and resync detection.
"""
...
class TestPostCompactionAppendUsesRawSuffix:
    ...
    compaction; sync drops the compacted prefix and appends only the new
    user message to the child snapshot."""
    ...
    async def test_appends_only_new_entry(self):
        ...
        # Only the raw window + new entry should be in child.messages — m1, m2 are
        # summarized into ancestor_summaries and must NOT reappear in messages.
        ...
        # No new branch — we extended the child snapshot in place.
        assert result.is_new_branch is False
        assert result.conversation.snapshot_uuid == "s-child"
        # Exactly one source_id was assigned (the bare follow-up).
    ...
    async def test_no_duplicate_messages_after_sync(self):
        ...
        would get re-appended to stored.messages, producing duplicates after
        re-sort. Verify the duplicate-injection doesn't happen."""
        syncer, redis, search = make_syncer()
        _setup_compacted_state(
            search,
            redis,
...
class TestSplitRunsBeforeResyncDetection:
    ...
    resync detector's `has_new_incoming` heuristic and the detector misfires.
    With the split, the detector sees only the raw suffix."""
    ...
    async def test_compacted_prefix_does_not_trigger_false_resync(self):
        ...
        un-acked bot. Client echoes the prefix + raw tail + new user message.

        Without the split: `_detect_unacknowledged_bot_messages` would see
        partial entries the stored child doesn't know about (m1, m2) and
        could plausibly misfire. With the split, the detector sees only
        the raw suffix, finds no trailing bot, and lets the flow continue.
        """
        syncer, redis, search = make_syncer()
        _setup_compacted_state(
            search,
...
class TestCaseBHelperAssignsSourceIds:
    ...
    Conversation from full incoming. Bare entries get source_ids assigned;
    handshake's source_id_assignments must carry the right client_index.

    The realistic Case B scenario is "user edits a message that was compacted
    away": the edit creates a new client node with no source_id, the active
    path is shorter than the compacted prefix, length mismatch routes to
    Case B."""
    ...
    async def test_edit_of_compacted_message_creates_fresh_conversation(self):
        ...
        # User edits u1: the active path is [u1', new-typed-message]. Both
        # client nodes have no source_id (edit + fresh type). Prefix split
        # fails on length (incoming=2 < raw_start=2... wait, also fail on
        # source_id mismatch since incoming has None). Either way, routes to
        # Case B.
        incoming = [
    ...
    async def test_source_id_mismatch_in_prefix_routes_to_case_b(self):
        ...
        entries to potentially match the compacted prefix in length, but at
        least one source_id is fabricated. Split fails on the source_id check
        and routes to Case B."""
        syncer, redis, search = make_syncer()
        _setup_compacted_state(
            search,
            redis,

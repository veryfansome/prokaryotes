...
`load_assistant_index` walks every conversation doc for the `conversation_uuid`,
including compacted ancestors (whose `messages_json` is preserved). The cache
in Redis (`assistant_index:{conversation_uuid}`) is refreshed on every bot
commit via `refresh_assistant_index_with`.
"""
...
class TestLoadAssistantIndex:
    ...
    async def test_indexes_bot_messages_across_dag(self):
        ...
        # Indexes every bot reachable in the DAG — b1, b2 (from compacted parent),
        # b2 (duplicated on child — same hash), b3.
    ...
    async def test_redis_hit_short_circuits_es(self):
        ...
        syncer, redis, search = make_syncer()
        # Prime the cache directly; do NOT seed ES.
        await redis.set(
            "assistant_index:c-1",
            json.dumps({"1.000002": _hash("b1")}),
        )
        # No conversation docs exist in ES — if the cache hit fails to short-circuit,
...
class TestValidateAssistantMessages:
    ...
    async def test_case_b_retry_with_compacted_ancestor_assistant_accepted(self):
        """Case B path: client echoes assistant `source_id`s that live only in
        the compacted parent's retained `messages_json`. The DAG-scoped lookup
        hits the parent, validation passes."""
        syncer, _redis, search = make_syncer()
        ...
        # Client edits something that requires referencing the compacted bot.
        # Validation must accept "1.000002" because the parent's messages_json
        # still has it.
        await syncer.validate_assistant_messages(
            "c-1",
            [

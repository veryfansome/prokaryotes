...
Under the unified model, ancestor summaries are NOT re-fed into the
summarization LLM call; they're already accumulated on the parent and carry
forward to the new compacted child verbatim. The summarization input is the
pre-tail messages projected, with an explicit summarization prompt as the
final user message, and `instruction=None`. Live-window bodies are stripped
from pre-tail TurnExecutions before projection so current file contents don't
...
def _make_harness() -> tuple[WebHarness, FakeAnthropicClient]:
    """Build a WebHarness with a recording fake LLM. Bypasses init() — we
    only need `_summarize_and_compact` and llm_client wiring."""
...
def _make_prep_with_pre_tail_turns(pre_tail_turns: dict) -> _CompactionPrep:
    """Build a _CompactionPrep with the given pre_tail_turns. pre_tail_messages
    are a bot message per bot_id in pre_tail_turns, plus a leading user msg."""
...
async def test_summarize_strips_live_window_bodies_from_pre_tail():
    """Live windows in pre-tail TurnExecutions must be replaced with the
    placeholder so the LLM doesn't fossilize current file contents into
    `ancestor_summaries`."""
...
async def test_summarize_uses_only_committed_turn_items():
    """The summarization input is built from `pre_tail_turns` items (committed
    function_call / function_call_output entries). Transient narration that
    never reaches `on_committed_turn_item` is structurally absent."""
    from tests.unit_tests._builders import conversation
    ...
    # No item should be a function_call without a matching output, and no
    # message item should be narration (we never seeded narration in the turn).
...
async def test_summarize_appends_summarization_prompt_as_user_message():
    """The final item in the summarization input is the summarization prompt
    as a `user` message."""
    from tests.unit_tests._builders import conversation
...
async def test_summarize_passes_no_instruction():
    """The summarization call passes `instruction=None`. Guards against
    accidentally re-feeding ancestor summaries via the system/developer slot."""
    from tests.unit_tests._builders import conversation
...
async def test_summarize_does_not_inject_ancestor_summaries_into_items():
    """The snapshot's `ancestor_summaries` carry forward to the child as
    storage state; they must NOT be projected into the summarization input."""
    from tests.unit_tests._builders import conversation

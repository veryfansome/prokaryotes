...
The split runs before resync detection and reconcile when stored has
`raw_message_start_index > 0`. It reconstructs the compacted-away prefix from
the parent chain and matches it against the first N non-deleted entries of
incoming. Match → strip the prefix; mismatch → caller routes to Case B.

Comparison is `(source_id, author_id, content)` — same-content / different
source_id routes to Case B (preserves source-ID-as-identity invariant).
"""
...
def _setup_post_compaction_state(search_client, *, parent_messages):
    """Build a compacted-parent + post-compaction-child pair, store both in the
    fake search client. Returns the child Conversation (the syncer's `stored`).
    """
    parent = conversation(
        *parent_messages,
    ...
    # Child: post-compaction tail. raw_message_start_index counts non-deleted
    # parent messages that were summarized away.
...
class TestMatchingPrefixStripsCleanly:
    """Happy path — incoming has prefix + raw window + new entry; split returns
    the raw-window suffix only."""
    ...
    async def test_returns_raw_suffix(self):
        ...
        # Parent had 4 messages; child has the last 2 ([m3=u2, m4=b2]).
        # raw_message_start_index=2. Client echoes all 4 + a new user message.
        partial = [
...
class TestSourceIdMismatchRoutesToCaseB:
    """Content matches but source_id differs — must route to Case B to preserve
    the source-ID-as-identity invariant."""
...
class TestWalksMultiCompactionChain:
    """Three-deep chain: root → child1 (compacted away m1..m4) → child2
    (compacted away m1..m6). Split for child2 should reconstruct the first 6
    messages from the global history walking through child1."""

...
class TestComputeBoundaryHash:
    ...
    def test_payload_includes_author_id_not_role(self):
        """Two messages identical in content but different authors must hash differently —
        role is no longer stored; author_id replaces it in the payload."""
...
class TestComputeTailHash:
    ...
    def test_multi_author_treated_uniformly(self):
        """`compute_tail_hash` takes content of any non-bot author. Two authors at the
        same source_id sequence with the same contents produce the same tail hash."""
        with_alice_and_bob = [
            msg("1", "alpha", author_id="u-alice"),

...
async def test_intermediate_narration_not_committed():
    ...
    item. Intermediate narration in a tool-use round is transient; only the
    final assistant text reaches on_final_assistant_message."""
...
async def test_stream_turn_yields_text_and_continues_after_tool_callback():
    """End-to-end tool-use round → final-text round produces the right chunk sequence
    and on_final_assistant_message only sees the final round's text."""
    callback = DummyToolCallback()
    client = _make_client(
        [
            FakeStreamContext(
    ...
    # The second call (after tool dispatch) sees the projected user message, then
    # the assistant tool_use block, then the tool_result. No transient narration text.
...
async def test_stream_turn_emits_progress_message_for_ndjson_tool_rounds():
    """Mid-tool-use narration emits as progress_message NDJSON (not text_delta)
    and does NOT call on_committed_turn_item with a message item."""
    callback = DummyToolCallback()
    client = _make_client(
        [
            FakeStreamContext(
...
async def test_stream_turn_passes_correct_params():
    """The provider call is built from `items` + `instruction` + `tool_callbacks`
    + `reasoning_effort`. ProjectedItem(role=system) reaches the LLM via the
    `instruction` kwarg, not as a wire-format user/assistant message."""
    callback = DummyToolCallback()
    client = _make_client(
        [

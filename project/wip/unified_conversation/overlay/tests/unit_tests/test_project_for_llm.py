...
class TestLiftedItems:
    ...
    def test_lifted_items_skipped_when_anchor_is_later_bot(self):
        ...
        # (since first bot has no turn, it stays plain assistant).
        assert types == [
            "message",  # user q
            "message",  # first bot
            "message",  # user q2
    ...
    def test_no_lift_when_anchor_none(self):
        ...
        items = project_for_llm(c)
        # No insertion point → no lifted items emitted (will be cleaned up by compactor's invariant
        # in practice, but the projection itself stays defensive)

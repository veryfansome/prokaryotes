...
`count_turns` operates on `list[TurnItem]` + a `had_final_assistant` boolean.
`run_task` consumes a `ScriptRunResult` and produces two artifact files
(`conversation.json`, `turn_execution.json`) instead of the legacy
...
def fake_harness(monkeypatch) -> FakeScriptHarness:
    """Install FakeScriptHarness as the make_script_harness target."""

...
Web (default `ConversationSyncer._apply_result`): edit, delete, divergence
all branch via `_apply_divergence`. The parent snapshot stays intact.

Slack (`SlackConversationSyncerMixin._apply_result`): edit / delete / divergence
mutate the stored snapshot in place. Delete also triggers tombstone re-keying
of the bot's `TurnExecution` and `lifted_anchor_source_id`.
"""
...
class TestWebBranchesOnEdit:
    """Web default: an edit to a stored message produces a new sibling snapshot;
    the parent snapshot stays intact with its original content."""
...
class TestWebBranchesOnDelete:
    """Web default: regenerate (omitting the trailing assistant message)
    produces a new sibling snapshot. The parent retains the deleted bot
    in non-tombstoned state."""
...
class TestSlackInPlaceDelete:
    """Slack: `message_deleted` sets the tombstone in place AND re-keys the
    bot's `TurnExecution` to the next non-tombstoned bot in the same run."""
...
class TestSlackDivergenceOverwrites:
    ...
    async def test_divergence_overwrites_in_place(self):
        """Slack reconciling against a stored snapshot that drifted — the
        recovery action is overwrite in place, not branch."""
        syncer, redis, search = _make_slack_syncer()
        stored = conversation(
            msg("1.000000", "u1-old"),
...
class TestTombstoneRekey:
    """`_rekey_for_tombstone` and `_next_non_tombstoned_bot_in_run`: the bot
    run walking + ES re-key behavior the Slack delete path uses."""

    @pytest.mark.asyncio
    async def test_rekey_single_bot_run_orphans_turn_execution(self):
        """Bot run of one (single bot reply after a user message): tombstoning
        it leaves no replacement, so the `TurnExecution` is deleted and the
        anchor (if it pointed at this bot) falls to `None`."""
        ...
        assert search.delete_turn_execution_calls == ["1.000001"]
    ...
    async def test_rekey_chain_promotes_next_bot(self):
        """Multi-post bot run (2 bots between user messages): tombstoning the
        first bot moves its `TurnExecution` to the second bot's source_id."""
        syncer, _redis, search = _make_slack_syncer()
        stored = conversation(
            msg("1.000000", "u1"),
        ...
        assert search.rekey_turn_execution_calls == [("1.000001", "1.000002")]
        assert search.delete_turn_execution_calls == []
        assert "1.000001" not in search.turn_executions
        assert "1.000002" in search.turn_executions

    @pytest.mark.asyncio
    async def test_rekey_full_run_tombstone_orphans(self):
        """Bot run of two with both already tombstoned by the time the helper
        runs: no replacement → `TurnExecution` deleted, anchor → None."""
        syncer, _redis, search = _make_slack_syncer()
        stored = conversation(
            msg("1.000000", "u1"),
            bot_msg("1.000001", "b1-part-a"),
    ...
    def test_user_tombstone_no_rekey(self):
        """Tombstoning a user message — `_next_non_tombstoned_bot_in_run`
        returns None because the deleted message isn't a bot (no run to walk)."""
        stored = conversation(
            msg("1.000000", "u1"),
            bot_msg("1.000001", "b1"),
            msg("1.000002", "u2"),
        ...
        # Walk from a user source_id — there's no bot run *forward* (the
        # next entry is u2, breaking the run), and no bot *backward* before
        # the user either. Returns None.
    ...
    def test_rekey_skips_already_deleted_bots_in_run(self):
        """Bot run of three with middle already tombstoned: tombstoning the
        first re-keys past the tombstoned middle to the third bot (membership
        includes the tombstoned middle but selection skips it)."""
        stored = conversation(
            msg("1.000000", "u1"),
            bot_msg("1.000001", "b1-part-a"),
            bot_msg("1.000002", "b1-part-b"),
    ...
    async def test_rekey_bot_without_turn_execution_no_op(self):
        """Tombstoning a non-owner bot (a continuation post that never had
        its own `TurnExecution` — only the first post in a run owns one)
        doesn't touch ES."""
        syncer, _redis, search = _make_slack_syncer()
        stored = conversation(
            msg("1.000000", "u1"),
        ...
        assert search.delete_turn_execution_calls == []
        assert search.rekey_turn_execution_calls == []
        assert "1.000001" in search.turn_executions

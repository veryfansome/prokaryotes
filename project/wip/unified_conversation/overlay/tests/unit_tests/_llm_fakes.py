...
Substitutes at the harness-contract level: `stream_turn` accepts the new
`(items: list[ProjectedItem], instruction: str | None, ...)` signature with
`on_committed_turn_item` / `on_final_assistant_message` callbacks. Intermediate
narration in tool-use rounds is emitted as `progress_message` NDJSON events
but NEVER passed to `on_committed_turn_item` (transient-narration invariant).

Recording hooks (`stream_turn_calls`, `complete_calls`) let tests assert on
exactly what reached the LLM layer.
"""
...
class _FakeLLMClientBase:
    ...
    async def complete(
        ...
        # Disambiguate summarization vs think:
        # - summarization (`WebHarness._summarize_and_compact`) calls with `instruction=None`
        # - think tool calls with a non-None `instruction` (its system prompt)
...
class FakeAnthropicClient(_FakeLLMClientBase):
    ...
    async def stream_turn(
        ...
        while self._round_cursor < len(self._script.rounds):
            ...
            # Tool-use round: intermediate narration is transient. Emit as
            # progress_message NDJSON, but NEVER call on_committed_turn_item with it.
            ...
            for tc in round_.tool_calls:
                ...
                if tool_callbacks and tc.name in tool_callbacks:
                    callback_tasks.append(asyncio.create_task(tool_callbacks[tc.name].call(tc.arguments, tc.call_id)))
...
class FakeOpenAIClient(_FakeLLMClientBase):
    ...
    Mirrors FakeAnthropicClient but emits events in OpenAI's order (tool_call
    before context_pct within a tool-use round). The on_committed_turn_item /
    on_final_assistant_message contract is identical.
    """

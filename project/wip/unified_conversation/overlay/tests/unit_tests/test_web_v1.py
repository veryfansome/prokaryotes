...
Covers WebBase's inheritance contract and the re-export surface that web_v1
maintains. Also exercises `finalize_turn`'s persistence side effects and
`validate_assistant_messages`'s four-branch guardrail at the helper level
(unit-scoped; the full FastAPI route is covered in Tier B).
"""
...
def test_webbase_inherits_new_shared_layers_without_new_abstractmethods():
    ...
    wb = WebBase("scripts/static")
    assert wb.html_dir.name == "html"


def test_web_v1_surface_re_exports_context_v1_helpers():
    """The web_v1 package re-exports key helpers from context_v1 + auth + db.
    These re-exports pin a stable import surface for downstream callers."""
...
async def test_validate_assistant_messages_noop_when_no_assistant_entries():
    ...
    # All user entries — should not raise even with no index.
...
async def test_finalize_strips_system_message_and_persists_to_redis_and_es():
    ...
    involvement) — the persisted state is only user/bot messages from the
    conversation. System instructions never enter the conversation."""
    redis = FakeRedis()
    search = FakeSearchClient()
    harness = _StubHarness(redis, search)
    conv = conversation(msg("1", "U1"), snapshot_uuid="s1")

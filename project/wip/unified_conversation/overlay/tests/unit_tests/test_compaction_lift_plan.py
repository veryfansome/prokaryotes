...
CAS-swap + ES integration tests live in the syncer/compactor integration suite
once a fake SearchClient is wired.
"""
...
class TestComputeLiftPlan:
    ...
    def test_anchor_is_first_bot_with_file_activity(self):
        """Anchor selects the *first* recency-tail bot whose TurnExecution has a
        file-tool annotation — placement is right before that turn's tool round."""
        plan = _compute_lift_plan(
            pre_tail_messages=[bot_msg("1", "b1")],
    ...
    def test_stale_pre_tail_pairs_excluded(self):
        ...
        # Stale pair not lifted; with no eligible lift, anchor falls through to None
        # (invariant: anchor=None iff lifted_turn_items == [])
        assert plan.lifted_turn_items == []
        assert plan.lifted_anchor_source_id is None

    def test_parent_lifted_items_carry_forward_through_compaction(self):
        """Transitive roll-forward: items lifted by an earlier compaction re-lift here
        if their path is still active and not superseded."""
        parent_lifted = [

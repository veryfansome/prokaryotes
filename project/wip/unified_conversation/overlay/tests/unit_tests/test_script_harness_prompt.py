...
`_build_instruction` returns a string passed via the `instruction` kwarg to
`stream_turn` (no longer a synthesized first item with a `role`). Provider-
specific role stamping (system vs developer) happens at wire-format time in
...
def test_script_harness_build_instruction_omits_summary_rules():
    """_build_instruction is the static helper that assembles the script-mode
    system prompt. It must include core instructions and non-interactive
    execution mode rules, but NOT the conversation-summary rules (script flow
    has no compaction)."""
...
async def test_script_harness_run_passes_instruction_to_stream_turn():
    ...
    client as the `instruction` kwarg to `stream_turn`. The user message
    becomes a single ProjectedItem(role="user")."""

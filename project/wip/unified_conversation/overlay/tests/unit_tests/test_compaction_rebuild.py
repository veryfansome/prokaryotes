...
Tier-3 load step: walks the snapshot DAG from the client's snapshot_uuid back
to a compacted ancestor whose boundary_hash matches a prefix of incoming, then
builds a Conversation rooted there. Safety: chain walk stops on
conversation_uuid mismatch, cycles, or missing intermediate docs.

Also covers HarnessBase.stream_and_finalize's handshake + compaction_pending
ordering and the duplicate-compaction-when-lock-held skip.
"""
...
async def test_rebuild_from_chain_assembles_two_generation_summaries_in_order():
    """Two compacted ancestors with valid boundaries → ancestor_summaries
    populated outer-first."""
    syncer, _, search = make_syncer()
...
async def test_rebuild_from_chain_does_not_inject_stale_summary_without_valid_boundary():
    """If an ancestor's boundary_hash doesn't match incoming, its summary is skipped.
    With no matching ancestor, fall back to a fresh Conversation (no ancestors)."""
    syncer, _, search = make_syncer()
    _seed_doc(
        search,
        _compacted_doc(
...
async def test_rebuild_from_chain_includes_only_summaries_up_to_matched_ancestor():
    """Three-generation chain where only the middle ancestor's boundary matches:
    ancestor_summaries includes the outer and middle, but NOT the inner (younger
    than the matched deepest-valid ancestor)."""
    syncer, _, search = make_syncer()
    p0_boundary = [msg("1", "U1"), bot_msg("2", "A1")]
    p1_boundary = p0_boundary + [msg("3", "U2"), bot_msg("4", "A2")]
...
async def test_rebuild_from_chain_uses_deepest_valid_compacted_ancestor():
    """When multiple ancestors have matching boundary_hashes, prefer the
    deepest (most-recent) — its raw_message_start_index is later, so less
    is replayed as raw window."""
    syncer, _, search = make_syncer()
    p0_boundary = [msg("1", "U1"), bot_msg("2", "A1")]
    p1_boundary = p0_boundary + [msg("3", "U2"), bot_msg("4", "A2")]
...
async def test_rebuild_from_chain_returns_fresh_when_no_snapshot_uuid():
    """A fresh conversation (no client-supplied snapshot_uuid) returns a fresh
    Conversation with no ancestors."""
...
class _StubHarness(HarnessBase):
    """Concrete HarnessBase for stream_and_finalize tests. Stubs out
    finalize_turn so we don't need to wire ES persistence."""
    ...
    def background_and_forget(self, coro):
        ...
        # Don't actually schedule — these tests just check emission ordering.
        coro.close()
...
async def test_stream_and_finalize_emits_handshake_first_and_compaction_pending_last():
    """First event = handshake carrying snapshot_uuid and source_id_assignments.
    Last event before completion = compaction_pending (when lock acquired)."""
    redis = FakeRedis()
...
async def test_stream_and_finalize_skips_duplicate_compaction_when_lock_held():
    """If `compaction_lock:` already exists (`nx=True` fails), no
    compaction_pending event and no background compaction kicked off."""
    redis = FakeRedis()
...
async def test_stream_and_finalize_omits_bot_message_when_no_final_text():
    """If the LLM stream aborts without producing final assistant text, no
    bot_message event is emitted and no finalize_turn happens (the client
    must not create an assistant node)."""
    redis = FakeRedis()
    search = FakeSearchClient()
    harness = _StubHarness(redis, search)
    sync_result = _make_sync_result()
...
async def test_stream_and_finalize_resync_closes_stream_after_handshake():
    """On resync, the handshake carries unacknowledged_bot_messages and the
    stream closes without invoking the LLM."""

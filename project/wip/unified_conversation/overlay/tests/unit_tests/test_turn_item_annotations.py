...
`prokaryotes_annotations` survives Python-side round-tripping but is structurally
excluded from the provider wire format (`ProjectedItem` has no such field, so
projection drops it before wire conversion). The wire-format converters in
`anthropic_v1` and `openai_v1` further enforce role conventions (`system →
developer` for OpenAI) and block-shape rules (no synthetic empty text prefix
before tool_use; correct grouping into role-bucketed messages).
"""
...
def test_to_anthropic_messages_does_not_synthesize_text_block_for_function_call():
    ...
    # Find the assistant message group; its content must be a single tool_use
    # block, NOT preceded by an empty text block.
...
def test_anthropic_full_conversion():
    ...
    via the `system` param), user/assistant text wrap in text blocks, function_call
    items become tool_use under an assistant message, function_call_output items
    become tool_result under a user message, consecutive same-role items coalesce."""
    items = [
...
def test_openai_full_conversion():
    ...
    developer message; system role on projected items rewrites to developer;
    function_call / function_call_output emit with type + call_id metadata."""
    items = [

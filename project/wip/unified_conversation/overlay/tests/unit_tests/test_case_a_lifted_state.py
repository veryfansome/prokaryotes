...
When a web edit/regenerate produces a new sibling snapshot inside the parent's
raw window, the child must:

- inherit `ancestor_summaries` and `raw_message_start_index` verbatim,
- filter parent's `lifted_turn_items` to paths still active in the *child's*
  raw window (shared prefix's TurnExecutions),
- pick `lifted_anchor_source_id` as the first non-tombstoned bot in the shared
  prefix with non-stale file-tool activity,
- preserve the invariant `anchor=None iff lifted_turn_items == []`.

Stub of `_active_paths_for_messages` previously caused this whole subsystem
to silently drop lifted state on every web branch. These tests guard the fix.
"""
...
class TestCaseABranchPreservesLifted:
    """Parent has lifted pair for `/a`; shared prefix's bot turn touches `/a`;
    new branch retains the lifted pair, anchored at the touching bot."""
    ...
    async def test_lifted_pair_carries_to_branch_when_path_active(self):
        ...
        # Web edit of u2 — regenerate from there; sends history up through u2.
        # Reconcile classifies as "delete" (omits b2), web policy branches.
        incoming = [
...
class TestCaseABranchDropsWhenPathNotActive:
    """Parent has lifted pair for `/a`, but the shared prefix has no `/a`
    activity — drop the lifted pair, anchor stays None."""
    ...
    async def test_no_shared_prefix_activity_drops_lifted(self):
        ...
        search.store_conversation_doc(parent)
        _store_redis_snapshot(redis, parent)
        # b1 touches /b; b2 touches /a. The shared prefix on regenerate-from-u2
        # is [u1, b1, u2] — only b1's turn matters.
        await search.put_turn_execution(
            TurnExecution(
                conversation_uuid=parent.conversation_uuid,
                bot_message_source_id="1.000001",
...
class TestCaseABranchMultipleWindowsPerPath:
    """Parent has two distinct lifted pairs for `/a` (different view ranges);
    branch retains both."""
...
class TestCaseAAnchorPicksFirstNonTombstonedBot:
    """When the shared prefix has multiple bots with file activity, the anchor
    is the FIRST non-tombstoned one (preserving today's placement rule)."""
    ...
    async def test_anchor_is_first_qualifying_bot(self):
        ...
        # Edit u3 — shared prefix = [u1, b1, u2, b2]. Both b1 and b2 are in the
        # shared prefix; b1 has no file activity, b2 has /a activity.
        incoming = [
            IncomingMessage(role="user", content="u1", source_id="1.000000"),
...
class TestCaseAInvariantHoldsWithNoSharedBots:
    """No shared bots (divergence right at the start): no shared turns to
    load, no active paths, lifted state is dropped per the invariant."""

...
class TurnItem(BaseModel):
    ...
    Replaces the tool-call subset of the old `ContextPartitionItem`. The final
    assistant message lives on the `Conversation`, not here; intermediate
    assistant narration is transient (see the transient-narration invariant in
    the design doc) and is never persisted.
    """
...
class TurnExecution(BaseModel):
    ...
    Holds only `function_call` / `function_call_output` items. Multi-post bot
    turns (Slack) are owned by the first post in the consecutive bot run.
    """
...
class Conversation(BaseModel):
    ...
    One snapshot per node in the snapshot DAG. A `conversation_uuid` is the
    dialogue identifier; many `snapshot_uuid`s share a `conversation_uuid` —
    linear children via compaction, sibling branches via edit/regenerate.
    """
...
class NormalizedMessage(BaseModel):
    """What `reconcile` operates on. Built by the syncer from incoming surface
    payload + session info. Storage-side flags (`deleted`, `edited`) are not
    carried on incoming."""
...
def compute_boundary_hash(messages: Iterable[ConversationMessage]) -> str:
    """Stable hash over non-deleted messages. Payload is `{author_id, content}` —
    role is no longer stored; it's derived at projection time."""
...
def conversation_message_items(messages: Iterable[ConversationMessage]) -> list[ConversationMessage]:
    """Non-deleted messages, in input order. Convenience for callers that want to
    skip tombstones without rewriting their iteration."""

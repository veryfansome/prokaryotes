...
class IncomingMessage(BaseModel):
    ...
    `source_id` is omitted on newly-authored messages (typed fresh, edit,
    regenerate); the syncer assigns one on first encounter. `author_id` and
    `display_name` are server-derived from session info and are not on the
    wire — see `NormalizedMessage`.
    """
...
class CompactionStatusResponse(BaseModel):
    ...
    `snapshot_uuid` is populated only when the swap committed a *direct child*
    of the polled-for `pending_snapshot_uuid` — that's the relabel target. In
    all other "done" outcomes (lock released without a commit, snapshot
    evicted, parent mismatch) the field is omitted; the client clears its
    indicator without relabeling.
    """
...
def _anthropic_input_schema(schema: object) -> object:
    ...
    Anthropic's custom tool schema accepts a narrower keyword set than
    OpenAI's. Integer properties currently reject `minimum`, so strip that
    keyword wherever the schema allows integers while leaving the original
    intact for providers that accept it.
    """

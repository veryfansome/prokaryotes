...
Owns its own `ProjectedItem → Anthropic message format` translation and its
own working buffer for the in-flight turn. Intermediate assistant narration
goes into the working buffer so subsequent rounds in the tool-use loop see
it, but is *not* committed to the `TurnExecution` (transient-narration
invariant). The final assistant text is delivered via `on_final_assistant_message`;
committed tool items via `on_committed_turn_item`.
"""
...
class AnthropicClient(LLMClient):
    ...
    async def stream_turn(
        ...
        Per-round translation: starts from the initial `items` and grows as
        tool_use/tool_result blocks are produced. Intermediate text narration
        flows into the buffer so the model sees its own scratch work between
        ...
        # Working buffer. Initialized from the projection; grows with tool_use/result
        # blocks. Discarded when stream_turn returns.
        working_messages = _items_to_anthropic_messages(items)
        # Aggregated text across rounds for fallback if `stop_reason` produces text
        # only on the final round (typical) — collected across all non-tool-use rounds.
        ...
        while True:
            ...
            # Intermediate text is transient — feed back to the model in subsequent
            # rounds but do NOT emit via on_committed_turn_item.
            round_output = "".join(round_text)
            if stream_ndjson and round_output:
                yield json.dumps({"progress_message": round_output}) + "\n"

            # Append the model's assistant turn (text + tool_use blocks) to the working buffer.
            assistant_blocks: list[dict] = []
            if round_output:
                assistant_blocks.append({"type": "text", "text": round_output})
            callback_tasks: list[asyncio.Task[TurnItem | None]] = []
            for block in response.content:
                ...
                if tool_callbacks and block.name in tool_callbacks:
                    callback_tasks.append(asyncio.create_task(tool_callbacks[block.name].call(args, block.id)))
                else:
            ...
            # Append tool_result blocks (all under one user-role message).
            tool_result_blocks: list[dict] = []
            for fc_item, result in zip([b for b in response.content if b.type == "tool_use"], results, strict=False):
                if result is None:
                    continue
                if on_committed_turn_item is not None:
...
def _items_to_anthropic_messages(items: list[ProjectedItem]) -> list[dict]:
    ...
    `type=message` items become text blocks; `function_call`/`function_call_output`
    become `tool_use` / `tool_result` blocks. Consecutive items of the same role
    coalesce into a single message — already enforced by `project_for_llm`'s
    same-role merge, but we re-group here so the wire shape is invariant under
    ad-hoc callers.
    """
    messages: list[dict] = []
    ...
    for item in items:
        if item.type == "message":
            if item.role == "system":
                # Should not reach the LLM client — instruction is the `system` param.
                # Drop defensively.
                flush()
                continue

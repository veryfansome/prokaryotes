...
Migrated to the unified conversation model. Synthesizes a single-turn
`Conversation` (one user message), projects it, drives `stream_turn` with
tool callbacks, and returns a `ScriptRunResult` carrying the final
`Conversation`, the produced `TurnExecution` (if any tool calls happened),
and the final assistant text.

Used by `EvalHarness` for fixtures and by `scripts/cli.py` for ad-hoc runs.
There is no persistence to Redis/ES — script runs are entirely ephemeral.
"""
...
# Fixed source_ids for the synthesized one-shot conversation. They don't need
# to be timestamps because the script flow never reconciles with a stored
# snapshot — projection sorts by source_id, so "0" < "1" is the only invariant
# that matters.
...
class ScriptRunResult:
    ...
    Replaces the old `ContextPartition` return shape. Callers (e.g. `EvalHarness`)
    can count tool calls off `turn_execution.items`, count think calls by
    filtering on `name == "think"`, and persist both the conversation and turn
    execution as separate JSON artifacts.

    `final_assistant_text` is empty if the turn aborted before producing a final
    assistant message (max-tool-call-rounds hit, stream error, etc.) — in that
    case `turn_execution.completed` is `False` too.
    """
...
class ScriptHarness:
    ...
    No persistence layer — the conversation lives entirely in memory and is
    returned to the caller via `ScriptRunResult`.
    """

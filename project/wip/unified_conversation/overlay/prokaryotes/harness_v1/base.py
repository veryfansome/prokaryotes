...
Replaces the `ContextPartition`-based base. The new flow centers on
`sync_conversation` returning a `SyncResult` carrying the snapshot plus any
`source_id` assignments and resync info. `stream_and_finalize` is the central
orchestrator: it emits the handshake (first event), drives the LLM stream,
commits the final assistant message to the `Conversation` and tool items to
the `TurnExecution`, and emits `bot_message` (last persistence-relevant event)
before yielding back any compaction signal.
"""
...
class _StreamFinalizationContext:
    """Internal handoff between the LLM client's stream_turn callbacks and
    `stream_and_finalize`'s commit step."""
...
class HarnessBase(ConversationCompactor):
    ...
    async def stream_and_finalize(
        ...
        *,
        ...
        - The **handshake** is always the first event, before any text/tool/progress.
          It carries the snapshot the bot will reply into (a fresh branch
          `snapshot_uuid` on divergence, the existing one otherwise), the
        ...
        - Otherwise, the LLM stream runs; intermediate `text_delta`/`tool_call`/
          `progress_message`/`context_pct` events flow through unchanged. The final
          assistant text is collected via the stream context object, and tool items
          are appended as they commit.
        - **`bot_message`** is emitted *exactly once*, after the final assistant text
          has been committed to the `Conversation`. If the turn fails before commit
          (LLM error, tool crash, stream abort), no `bot_message` is emitted.
        """
        ...
        # Run the LLM stream. The factory wires `on_committed_turn_item` and
        # `on_final_assistant_message` into the LLM client; we own the buffers here.
        ...
        if not final_text:
            # No final text produced — turn ended without a commitable bot message.
            # Do NOT emit bot_message; the client must not create an assistant node.
            return

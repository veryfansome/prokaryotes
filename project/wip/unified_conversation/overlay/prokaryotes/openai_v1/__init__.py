...
Owns its own `ProjectedItem → OpenAI Responses input` translation and its own
working buffer for the in-flight turn. Intermediate assistant narration is
transient (not emitted via `on_committed_turn_item`). The final assistant
text is delivered via `on_final_assistant_message`; committed tool items via
...
class OpenAIClient(LLMClient):
    ...
    async def stream_turn(
        ...
        while True:
            ...
            # Track function_call items dispatched this round so we can append
            # their outputs to the working buffer for the next round.
            round_function_calls: list[TurnItem] = []
            ...
            async for event in await self.async_openai.responses.create(  # type: ignore
                ...
                str_to_yield = await self._handle_event(
                    ...
                    round_function_calls=round_function_calls,
    ...
    async def _handle_event(
        ...
        *,
        ...
        on_committed_turn_item: Callable[[TurnItem], None] | None,
        ...
        if event.type == "response.output_text.delta":
            ...
            round_function_calls.append(fc_item)
            ...
            if tool_callbacks and event.item.name in tool_callbacks:
                callback_tasks.append(
                    asyncio.create_task(tool_callbacks[event.item.name].call(event.item.arguments, event.item.call_id))
                )
            else:
...
def _items_to_openai_input(items: list[ProjectedItem], instruction: str | None) -> list[dict]:
    """ProjectedItem → OpenAI Responses input list. Instruction is injected as a
    leading `developer` message (OpenAI's equivalent of `system`)."""
    result: list[dict] = []
    if instruction:

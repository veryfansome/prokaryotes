...
class ContextPartition(BaseModel):
    ...
    def to_openai_input(self) -> list[dict]:
        ...
        `prokaryotes_annotations` is internal harness metadata and is excluded from the
        ...
        result = []
        for item in self.items:
            item_dict = item.model_dump(exclude_none=True, exclude={"prokaryotes_annotations"})

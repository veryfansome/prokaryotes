...
def _strip_live_window_bodies(partition: ContextPartition) -> ContextPartition:
    ...
    - `CONFLICT` and `RANGE_ERROR` results keep their two header lines (which describe
      what the model tried and the failure mode) and have only the embedded
      `Current view ...` body — the actual file contents — replaced with the placeholder.
      That preserves the historical "model tried with stale revision" / "model passed an
      out-of-range edit" signal in the summary while keeping current content out.
...
def _strip_live_window_output(item: ContextPartitionItem) -> str:
    ...
    if output.startswith("CONFLICT ") or output.startswith("RANGE_ERROR "):
        # Keep diagnostic header lines (everything up to the embedded "Current view"
        # body) and replace only the body with the placeholder.
        head, sep, _ = output.partition("\nCurrent view")
        if sep:
            return head + "\n" + placeholder
    return placeholder
...
def _lift_active_live_windows(
    ...
    path is moved into the new tail immediately before the first item carrying a
    `file_tool.path` annotation. Original `call_id`s and arguments are preserved.
    ...
    inserting before the first annotated item slots lifted pairs after that leading user
    prefix while keeping them adjacent to the downstream activity that uses them.
    """
    # Build a call_id → idx map of recency-tail function_calls so when an annotated
    # function_call_output is the first annotated item, we can back up to its paired
    # function_call and insert before that — keeping the (call, output) unit intact.
    ...
    for idx, item in enumerate(recency_tail):
        ...
        if item.type == "function_call_output":
            ...
            insertion_idx = paired_call_idx if paired_call_idx is not None else idx
        else:
            insertion_idx = idx
    if not active_paths:
...
class WebBase:
            ...
            async with self.redis_client.pipeline() as pipe:
                while True:
                    try:
                        ...
                        augmented_tail = _lift_active_live_windows(pre_tail, recency_tail)
                        ...
                        swapped_partition = ContextPartition(
                            ...
                            items=augmented_tail + post_snapshot_items,
                        )
    ...
    async def get_highlight_css(self):
        return FileResponse(self.static_dir / "highlight.min.css")

    async def get_highlight_js(self):
        return FileResponse(self.static_dir / "highlight.min.js")
    ...
    async def get_markdown_it_js(self):
        return FileResponse(self.static_dir / "markdown-it.min.js")

    async def get_purify_js(self):
        return FileResponse(self.static_dir / "purify.min.js")
    ...
    def init(self):
        ...
        self.app.add_api_route("/highlight.min.css", self.get_highlight_css, methods=["GET"], include_in_schema=False)
        self.app.add_api_route("/highlight.min.js", self.get_highlight_js, methods=["GET"], include_in_schema=False)
        ...
        self.app.add_api_route("/markdown-it.min.js", self.get_markdown_it_js, methods=["GET"], include_in_schema=False)
        self.app.add_api_route("/logout", self.get_logout, methods=["GET"], include_in_schema=False)
        self.app.add_api_route("/purify.min.js", self.get_purify_js, methods=["GET"], include_in_schema=False)

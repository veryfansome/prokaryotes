...
```
[summary from P0] + [summary from P1] + [raw items from P2]
```

Summaries are prepended to the system (Anthropic) or developer (OpenAI) message. They are not injected as conversation turns, which means the model sees them as instructions and background context rather than as attributed exchanges.
...
**`compaction_pending`** — Emitted at the end of a response when a background compaction has been scheduled. The UI shows a non-blocking indicator. The indicator clears when a subsequent stream arrives carrying a different `partition_uuid`, confirming that the Redis swap completed and a new active partition is in use.
...
| `scripts/static/ui.js` | `getLastPartitionUuid`, `partition_uuid` event handling, `context_pct` fill indicator, compaction indicator show/clear logic |
| `tests/test_api_v1_models.py` | Unit tests for hash functions, `ContextPartition` sync, ancestor summary injection |
| `tests/test_web_v1.py` | Integration tests for `sync_context_partition` (all paths), `_compact_partition` atomic swap, fork and retry scenarios |
...
| `tests/test_openai_v1.py` | Tests for `context_pct` emission and `text_preamble` exclusion from compaction payload |

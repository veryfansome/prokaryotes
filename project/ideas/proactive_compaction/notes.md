...
The current compaction trigger is fixed: token threshold crossed → summarize pre-tail → swap into a child partition. There are at least two axes worth exploring beyond that.

## Possible triggers

- **Token threshold** (current).
- **File change.** Reconciliation detects a tracked file has been wholesale rewritten, making prior history about that file substantially less useful. Origin: surfaced during the file tool design at `project/wip/file-tool-tracked-context/`.
...
Each trigger needs criteria for "this should fire" that don't produce false positives.

## Possible methods

- **Summary-based** (current). Feed the pre-tail to an LLM, get a textual summary, replace the pre-tail with the summary in a child partition. Brute force and lossy: verbatim phrasing the user used, exact constraints, specific tool outputs that turn out to matter — all dissolved into prose.
...
This idea surfaced during the file tool design at `project/wip/file-tool-tracked-context/`, which introduces *live windows* that refresh in-place when on-disk content changes. The specific scenario that prompted it: a tracked file is wholesale rewritten externally; reconciliation refreshes earlier reads to show the new content; but prior edit records and the model's pre-tail reasoning still reference the old contents and line numbers. The token-threshold trigger may not fire for many more turns. A proactive trigger would compact closer to the moment the history actually became outdated.

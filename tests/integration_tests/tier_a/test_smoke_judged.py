...
Drive a conversation to compaction, then assert binary behavioral properties via `llm_judge_majority`. Run by hand;
each test costs real tokens on both the subject provider and the OpenAI judge.
"""
...
@pytest.mark.asyncio(loop_scope="session")
async def test_branch_isolation_after_retry_before_tail(web_harness, authed_client):
    """Retry below the compacted recency tail → fresh branch, no carryover summaries.

    Structural-only: asserts the persisted branch shape (empty `ancestor_summaries`,
    `raw_message_start_index == 0`, no `parent_snapshot_uuid`). Whether the model itself leaks across branches
    is a model-quality question, not a harness contract.
    """
    seeded: list[dict] = []
    for i in range(4):
        seeded.append(user_message(f"Seed turn {i + 1}: discuss item {i + 1} briefly."))
    seeded.append(user_message("My private codename is PHORBAS-QUANTA-93."))
    ...
    # Retry below the recency tail: 2 new user messages, no pre-compaction echo.
    payload = {
        "conversation_uuid": conversation_uuid,
        "snapshot_uuid": post_compaction_uuid,
        "messages": [
            user_message("Different topic entirely."),
            user_message("Reply with one short sentence."),
        ],
    }
    async with request_scope(web_harness):
        record = await post_chat_collect(authed_client, payload)

...
// messageTree is a `Map<nodeId, node>` to match the production data structure in `scripts/static/ui.js` (`const
// messageTree = new Map()`). Nodes also carry `children: number[]` and `activeChildId: id | null` so the
...
function makeTree(nodes) {
    ...
    // Wire children arrays from parentId back-references so tests don't have to spell out both sides of every link.
...
function fakeCreateNode(messageTree) {
    let counter = 100;
    // Mirror production `createMessageNode`: wire `children` / `activeChildId` on the parent and initialize
    // empties on the new node.
...
describe("applyHandshake", () => {
    ...
    it("preserves existing source_id but always restamps snapshot_uuid", () => {
        // Divergence: client sent a node with an existing source_id, server created a new branch. The handshake's
        // snapshot_uuid is authoritative even for nodes that already had source_ids.
    ...
    it("stamps snapshot_uuid even on nodes without assignments", () => {
        // Invariant: every submitted user node gets snapshot_uuid from the handshake, even if it already had a
        // server-assigned source_id from a prior turn.
...
describe("applyResyncHandshake — send-from-leaf", () => {
    it("reparents pending user under last reconstructed assistant (bidirectional)", () => {
        // Scenario: client previously acked u1@1.000001 and b1@1.000002. Just typed u2 locally as n3 (no source_id
        // yet). Server-stored has trailing b2@1.000003 (a multi-post continuation or follow-up bot) that the
        // client missed.
        const messageTree = makeTree([
        ...
        // The reconstructed bot is parented under the node whose source_id matches its parent_source_id — that's
        // n2 (b1), not n1 (u1).
        ...
        // Pending user n3 was detached from its old parent (n2) and reparented under the reconstructed bot — both
        // sides of the link.

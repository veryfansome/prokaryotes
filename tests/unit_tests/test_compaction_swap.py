...
Covers the swap step's structural properties: ancestor-summary accumulation, raw_message_start_index advance,
post-snapshot message carry-forward, WATCH contention retry, ES-write retry / abort paths, prefix-equality
skips, and the new `compaction_status:{pending}` relabel-target write.
"""
...
def _default_messages() -> list:
    """Long enough that pre_tail is non-empty with the default COMPACTION_RECENCY_TAIL=6: 12 messages →
    pre_tail has 6 of them."""
    return [
        msg("1.000000", "U1"),
        bot_msg("2.000000", "A1"),
        msg("3.000000", "U2"),
...
async def _setup(
    ...
    *,
    ...
    """Helper: build a compactor + fakes seeded with the active Redis snapshot and the snapshot's ES doc. If
    `active` is given, it replaces `snapshot` in Redis (simulating divergence). Always sets the compaction
    lock."""
...
async def test_compact_conversation_accumulates_ancestor_summaries_across_generations():
    """Each compaction concatenates the new summary onto the parent's ancestor_summaries list, preserving
...
async def test_compact_conversation_advances_raw_message_start_index_by_tail_offset():
    """The child's raw_message_start_index = parent's index + the count of non-deleted pre-tail messages (the
    tail offset)."""
...
async def test_compact_conversation_carries_forward_post_snapshot_messages():
    """Messages appended during compaction (after the snapshot was taken) are carried into the child
    snapshot."""
    snapshot = _make_snapshot()
...
async def test_compact_conversation_releases_lock_when_compact_fn_raises():
    """The compaction lock is always released, even when `compact_fn` raises. The relabel-target sentinel ("")
    is written so polling clients clear their indicator without trying to navigate."""
    snapshot = _make_snapshot()
    ...
    cached = Conversation.model_validate_json(await redis.get(f"conversation:{snapshot.conversation_uuid}"))
    assert cached.snapshot_uuid == "snap"  # active snapshot unchanged
...
async def test_compact_conversation_retries_redis_swap_on_watch_contention():
    """WATCH contention on the CAS swap triggers a retry; the second attempt commits successfully."""
    snapshot = _make_snapshot()
    execute_calls = []
...
async def test_compact_conversation_returns_early_for_empty_summary():
    """Empty summary returns BEFORE writing any compaction_status key — no sentinel, no CAS swap, no parent
    update. Lock still released via finally."""
    snapshot = _make_snapshot()
    compactor, redis, search = await _setup(snapshot)
...
async def test_compact_conversation_returns_early_with_no_status_write_when_pre_tail_empty():
    """When the recency tail consumes all messages (pre_tail empty), there's nothing to compact: return early
    without writing the status key."""
    ...
    assert summary_calls == []  # compact_fn never called
    assert await redis.exists(f"compaction_lock:{snapshot.conversation_uuid}") == 0
    assert await redis.get(f"compaction_status:{snapshot.snapshot_uuid}") is None
...
async def test_compact_conversation_skips_swap_when_active_prefix_changed():
    """If the active prefix diverges (different ancestor_summaries or raw_message_start_index), the swap is
    skipped and the sentinel is written."""
    snapshot = _make_snapshot()
...
async def test_compact_conversation_skips_swap_when_active_snapshot_uuid_changed():
    """If the active snapshot's snapshot_uuid changed (branch switch happened after the snapshot was taken),
    the swap is skipped."""
    snapshot = _make_snapshot()
...
async def test_compact_conversation_skips_swap_when_redis_conversation_missing():
    ...
    await redis.set(f"compaction_lock:{snapshot.conversation_uuid}", "1")
    search.store_conversation_doc(snapshot, is_compacted=False)
    # Conversation deliberately not seeded in Redis.
...
async def test_boundary_hash_stored_on_es_covers_full_parent_prefix():
    """The parent's `boundary_hash` post-update covers the full reconstructed ancestor prefix + the parent's
    own messages (not just the parent's own)."""
    p0_messages = [
        msg("1.000000", "U1"),
        bot_msg("2.000000", "A1"),
    ]
...
async def test_compact_conversation_aborts_before_redis_swap_if_child_persist_fails(monkeypatch):
    """If the child snapshot persist to ES fails through all retries, the CAS swap doesn't run; the active
    snapshot stays on the parent."""
...
async def test_compact_conversation_keeps_committed_child_reachable_when_parent_update_retries_exhausted(monkeypatch):
    """Parent-update retry exhaustion leaves the child reachable in ES but parent stays un-compacted. CAS swap
    already happened, so the Redis cache points at the new child."""
    monkeypatch.setattr(compaction_module, "_COMPACTION_SEARCH_WRITE_RETRY_DELAYS_SECONDS", (0, 0))
    snapshot = _make_snapshot()
...
async def test_compact_conversation_leaves_pending_child_when_cas_never_commits(monkeypatch):
    """If WATCH contention persists past the implicit retries (Redis swap races with branch switch), the
    pending child stays in ES with PENDING state; parent stays un-compacted."""
    monkeypatch.setattr(compaction_module, "_COMPACTION_SEARCH_WRITE_RETRY_DELAYS_SECONDS", (0, 0))
    snapshot = _make_snapshot()
...
async def test_messages_json_preserved_on_compacted_parent():
    """After commit, the parent's messages_json must remain populated (the DAG-scoped guardrail needs to walk
    the parent chain even after compaction)."""
    snapshot = _make_snapshot()
    compactor, redis, search = await _setup(snapshot)
...
async def test_compaction_status_written_to_redis_with_child_snapshot_uuid():
    """At commit time, compaction_status:{pending} → committed child's snapshot_uuid. The TTL matches
    conversation_cache_ex."""
    snapshot = _make_snapshot()
    compactor, redis, _ = await _setup(snapshot)

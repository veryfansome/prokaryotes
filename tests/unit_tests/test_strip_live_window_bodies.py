...
Replaces every live-window `output` with a placeholder so the summarization LLM call can't fossilize current file
contents into `ancestor_summaries`. Diagnostic headers (CONFLICT, ALREADY_EXISTS, RANGE_ERROR, RANGE_TRUNCATED) are
preserved because the model needs to react to them. Edit records, tombstones, and non-live items pass through unchanged.
...
def test_strip_live_window_bodies_handles_empty_current_view_marker():
    """Output starting directly with `Current view` (no diagnostic lines)
    collapses entirely to the placeholder."""
...
def test_strip_live_window_bodies_strips_inactive_paths_too():
    ...
    stripped regardless of whether its path is currently in the active set. The lift planner handles active-path

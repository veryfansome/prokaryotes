...
The reconcile-tier (Redis/ES/chain rebuild) tests live with the full syncer suite once we have a fake
SearchClient + Redis. These exercise just the helpers that don't need infra.
"""
...
class TestConversationCanFollowClient:
    ...
    def test_matching_parent_snapshot_uuid(self):
        """A cached child snapshot can serve a client whose own pointer is the parent — the relabel hasn't
...
class TestDetectUnacknowledgedBotMessages:
    ...
    def test_no_resync_when_last_shared_source_id_has_changed_content(self):
        """If the last shared source_id has different content between stored and
        incoming, it's an EDIT, not a resync anchor. Reconcile + `_apply_divergence`
        should branch the snapshot; a resync handshake here would skip the LLM
        call and lose the edit."""
        stored = conversation(
            msg("1", "u1-original"),
            bot_msg("2", "b1"),
        )
        # Incoming has u1's source_id but with edited content, plus a new follow-up.
        partial = [_partial("u1-edited", "1"), _partial("follow-up", None)]
        result = _detect_unacknowledged_bot_messages(stored, partial)
        assert result == []

    def test_no_resync_when_last_shared_source_id_has_changed_author(self):
        """A shared source_id that rebinds to a different author is divergence,
        not a resync anchor."""
        stored = conversation(
            msg("1", "u1"),
            bot_msg("2", "b1"),
        )
        partial = [_partial("u1", "1", author_id="someone-else"), _partial("follow-up", None)]
        result = _detect_unacknowledged_bot_messages(stored, partial)
        assert result == []

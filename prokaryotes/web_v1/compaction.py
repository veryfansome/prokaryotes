...
Reads only from Redis. The compactor writes `compaction_status:{pending_snapshot_uuid}` at CAS-commit step with
either the child `snapshot_uuid` (relabel target) or an empty-string sentinel (no relabel target — lock released
without a commit).

Returning `done=true` without `snapshot_uuid` is correct for both the empty-sentinel case and the
long-idle-past-TTL case; the client clears its indicator without relabeling either way.
"""
...
class CompactionStatusHandler(ABC):
    async def get_compaction_status(
        ...
        # Lock released — compactor either committed a child or skipped (empty summary, prefix divergence,
        # live→stale tombstone). The compactor writes the relabel target (or "" sentinel) to

...
3. `post_chat()` assembles the system/developer `ContextPartitionItem` (tool guidance, runtime context, ancestor summaries per provider — see below) and inserts it at `context_partition.items[0]`.
...
**Anthropic**: do not include `ancestor_summaries` in the system message you assemble. `ContextPartition.to_anthropic_messages()` — called inside `LLMClient.stream_response()` — extracts and prepends them to the system string automatically. Including them manually would duplicate them.

**OpenAI**: prepend `ancestor_summaries` explicitly to the developer message parts before assembling the instruction block. `to_openai_input()` does not inject them.
...
# OpenAI — summaries injected manually

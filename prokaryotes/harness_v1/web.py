...
1. `sync_conversation` returns a `SyncResult` carrying the snapshot to use (a fresh branch on divergence; the
   existing one otherwise), any server-assigned source_ids, and on stream-loss recovery,
   `unacknowledged_bot_messages`.
2. If resync: `stream_and_finalize` emits the handshake and closes — no LLM call.
3. Otherwise: load `TurnExecution`s for the raw window's bot messages, refresh tracked files against the unified
   view, build tools/instruction/projection, then stream. Tool items commit via `on_committed_turn_item`; final
   assistant text via `on_final_assistant_message`. Bot message gets a fresh server-assigned `source_id` and is
   committed last, before `bot_message` is emitted.
"""
...
class WebHarness(WebBase):
    ...
    Both providers receive the unified `instruction` string from `_build_instruction_parts`; per-provider
    wire-format wrapping happens inside each client.
    """
    ...
    async def post_chat(
        ...
        # DAG-scoped guardrail: reject POSTs that fabricate or rewrite bot messages. Runs before
        # `sync_conversation` so `_partially_normalize` can't quietly accept hostile `role="assistant"` entries by
        # mapping them to `bot_author_id`.
        try:
    ...
    async def _dispatch_turn(
        ...
        *,
        ...
        """Wire `sync_result` through `stream_and_finalize`. On resync, the latter emits just the handshake and
        returns — no further setup needed."""
        ...
        # Load TurnExecutions for the raw window's bot messages so projection can interleave tool items, and so
        # the file-tool view sees them.
        bot_source_ids = [
        ...
        active_turn = TurnExecution(
            ...
            # The bot's source_id is assigned at finalize time; this placeholder is ignored when active_turn is
            # persisted under the real source_id.
        ...
        # List so `on_usage` can mutate the flag in place across closures. A plain bool passed to
        # `stream_and_finalize` would be an immutable copy that the callback's update couldn't reach.
    ...
    def _build_instruction_parts(
        ...
        *,
        ...
        # The instruction message is the place where ancestor summaries enter the LLM call — projection's
        # same-role merge leaves them in the conversation messages for Anthropic's `system` placement and OpenAI's
        # `developer` message uniformly.
    ...
    async def _summarize_and_compact(
        ...
        *,
        ...
        """Build the summarization input and make a non-streaming LLM call for the summary. Live-window bodies are
        stripped from pre-tail TurnExecutions so current file contents don't fossilize into ancestor summaries.
        """
        ...
        # Build a partial Conversation containing only the pre-tail messages — that's what gets summarized.
...
def _assign_bot_source_id(conversation: Conversation) -> str:
    ...
    Same `seconds.microseconds` format as the syncer's user-message assignment, bumped to be strictly greater than
    the latest source_id in the conversation.
    """
    last_id = max(
...
async def _empty_stream():
    """No-op generator for the resync path; `stream_and_finalize` has already emitted the handshake."""

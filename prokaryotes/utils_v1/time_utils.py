...
def local_dt_str(dt: datetime, time_zone: str | None = None, fmt: str = '%Y-%m-%d %H:%M') -> str:
    return dt.astimezone(tz(time_zone)).strftime(fmt)


def local_now(time_zone: str | None = None) -> datetime:
    return utc_now().astimezone(tz(time_zone))


def local_now_str(time_zone: str | None = None, fmt: str = '%Y-%m-%d %H:%M') -> str:

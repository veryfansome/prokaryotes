...
Two indices:
- `conversations` — one doc per `Conversation` snapshot (compaction children and branch siblings alike). Retains
  `messages_json` even after `is_compacted=True` so the DAG-scoped assistant-message guardrail can read compacted
...
def _turn_execution_doc_id(conversation_uuid: str, bot_message_source_id: str) -> str:
    """Composite ES `_id` for `TurnExecution` docs.

    `bot_message_source_id` is a `seconds.microseconds` timestamp string that's only monotonic *within* one
    conversation. Two concurrent bot replies in different conversations can collide at the microsecond. Scoping the
    doc id by `conversation_uuid` makes the key globally unique.
    """
...
def _default_boundary_fields(conversation: Conversation) -> dict[str, object]:
    ...
    Used at non-compaction writes. The compactor overrides these with full-prefix-included values at the swap step —
    see `ConversationCompactor`.
    """
...
class ConversationSearcher(ABC):
    ...
    async def delete_turn_execution(self, conversation_uuid: str, bot_message_source_id: str) -> None:
        """Sweep — used when a `TurnExecution`'s owning bot run is fully tombstoned or the snapshot is compacted
        away."""
        doc_id = _turn_execution_doc_id(conversation_uuid, bot_message_source_id)
        try:
            await self.es.delete(index=TURN_EXECUTIONS_INDEX, id=doc_id)
        except Exception:
            logger.warning("Failed to delete turn-execution %s", doc_id, exc_info=True)
    ...
    async def rekey_turn_execution(self, conversation_uuid: str, old_id: str, new_id: str) -> None:
        ...
        ES has no native id rename — implement as `index(new_id) + delete(old_id)`. Called when a bot
        `ConversationMessage` that owns a `TurnExecution` is tombstoned and the next non-tombstoned bot in the same
        consecutive run becomes the new owner.

        Silent no-op if the old doc doesn't exist (the run-walker may pass us a speculative re-key for an entry that
        was never persisted yet), or if `old_id == new_id` — the `index(new)` + `delete(old)` sequence would
        otherwise delete the doc it just wrote.
        """
        if old_id == new_id:
            return
        old_doc_id = _turn_execution_doc_id(conversation_uuid, old_id)
        new_doc_id = _turn_execution_doc_id(conversation_uuid, new_id)
        try:
            result = await self.es.get(index=TURN_EXECUTIONS_INDEX, id=old_doc_id)
        except Exception:
            return
        doc = result["_source"]
        ...
        try:
            await self.es.index(index=TURN_EXECUTIONS_INDEX, id=new_doc_id, document=doc)
            await self.es.delete(index=TURN_EXECUTIONS_INDEX, id=old_doc_id)
        except Exception:
            logger.warning(
    ...
    async def find_all_conversation_docs(self, conversation_uuid: str) -> list[dict]:
        """Return every conversation doc for `conversation_uuid` (compacted ancestors and branch siblings alike).
        Used by the DAG-scoped assistant-message guardrail to populate the known-source_id index.

        Cap is generous (10k) — a single dialogue's DAG sits in the tens of snapshots at most under realistic
        usage."""
    ...
    async def find_conversation_by_tail_hash(self, conversation_uuid: str, tail_hash: str) -> dict | None:
        """Heuristic fallback for clients that didn't persist their `snapshot_uuid`: given a tail_hash computed from
        the last N non-bot messages, locate the nearest compacted ancestor without a full chain walk."""
        response = await self.es.search(
            index=CONVERSATIONS_INDEX,
            query={
                "bool": {
    ...
    async def put_conversation(
        ...
        *,
        ...
        `refresh` defaults to `False`: conversation writes are eventually consistent for search, and the hot path
        (every turn's sync + finalize) must not block on an ES refresh. Pass `refresh="wait_for"` only where a search
        must see the write immediately — see the compaction-child commit in `ConversationCompactor`.
        """
    ...
    async def put_turn_execution(self, turn: TurnExecution) -> None:
        ...
        # `wait_for`: the next turn's projection and background compaction both read TurnExecutions via batch search,
        # which only sees post-refresh state. Without this, follow-up turns within ES's 1-second refresh window
        # project a conversation that's missing this turn's tool items.
        await self.es.index(
            index=TURN_EXECUTIONS_INDEX,
            id=_turn_execution_doc_id(turn.conversation_uuid, turn.bot_message_source_id),
            document=doc,
            refresh="wait_for",
        )
    ...
    async def update_conversation(self, snapshot_uuid: str, *, refresh: str | bool = False, **fields) -> None:
        """Patch fields on a snapshot doc.

        `refresh` defaults to `False`. The compaction-child commit passes `refresh="wait_for"` so a post-compaction
        cold-Redis `_rebuild_from_chain` can find the new committed child via `find_latest_active_child`.
        """

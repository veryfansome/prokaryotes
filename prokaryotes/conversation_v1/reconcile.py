...
Returns a classification + operation list; the syncer interprets per surface (Slack overwrites in place, web
...
def reconcile(
    ...
    Preconditions:
    - Every incoming entry is a fully-populated `NormalizedMessage`. The syncer translates raw surface payloads —
      web `IncomingMessage` + session, or a Slack event — into `NormalizedMessage` and assigns `source_id`s to any
      newly-authored entries before calling this function.
    - `source_id` is sortable; lexicographic order equals chronological under the `seconds.microseconds` format.

    Classifications:
    - `match`: no operations.
    - `append`: every stored non-deleted source_id appears in incoming with matching content, and incoming has
      additional source_ids at the end.
    - `edit`: every stored non-deleted source_id appears in incoming, but at least one content differs (Slack
      `message_changed`).
    - `delete`: stored has non-deleted source_ids that are not in incoming, and incoming has no fresh source_ids
      and no content changes.
    - `divergence`: anything else — typically incoming has new source_ids that didn't exist in stored AND omits
      stored source_ids (web edit/regenerate), or mixes edits/deletes/appends in a non-trailing way.

    `divergence_point_index` indexes into `stored.messages` (including deleted entries) at the position immediately
    after the longest matching prefix — the natural rooting point for a child snapshot.
    """
...
def _index_after_prefix(stored_messages, shared_prefix_source_ids):
    """Return the index in `stored_messages` (including tombstones) just after the last shared source_id. Returns 0
    if no prefix is shared, or `None` if `stored_messages` is empty."""

...
Single place where role assignment, multi-author prefixing, lifted-window placement, and consecutive same-role
text merging happen. Everything else in the system is role-agnostic.
"""
...
def project_for_llm(
    ...
    Lifted items (carried across compaction) emit immediately before the anchor bot turn's tool round, preserving
    today's lift placement (after any leading user prefix, adjacent to the first relevant file activity).
    """
...
def current_turn_items(
    ...
    Tools that need to see durable state lifted across compaction must read from this view rather than from a
    single source.
    """

...
The lift-related helpers (`lift_active_live_windows`, `_tool_round_start_index`, `recency_tail_items`,
`_message_count_before_item_index`, `items_equal_mod_live_windows`) are removed — that selection now lives in
`ConversationCompactor._compute_lift_plan` and operates on `TurnExecution`s rather than a flat item list. The new
compactor's CAS prefix check (`_messages_match_prefix`) operates on `Conversation.messages`, which are append-only
with tombstone flags — no `mod_live_windows` carve-out needed.
"""
...
def _is_unstable_coverage(item: TurnItem) -> bool:
    """`ALREADY_EXISTS`, `CONFLICT`, `RANGE_ERROR` carry decision-relevant state the model still needs to react to
    before consuming the embedded view. `RANGE_TRUNCATED` is intentionally excluded — its embedded view is real
    coverage for the returned span."""
    output = item.output or ""
...
def _refresh_live_windows(
    ...
    """Re-render every live window for `path` against `text`/`revision`. Items already at `revision` are left alone.
    Returns the number of items actually rewritten."""
    refreshed_count = 0
    for item in items:
...
def strip_live_window_bodies(items: list[TurnItem]) -> list[TurnItem]:
    """Return a deep copy of `items` with each live-window `output` rewritten so the summarizer cannot fossilize
    current file contents into `ancestor_summaries`.

    The compactor's summarization pass calls this on pre-tail TurnExecutions before projecting them for the summary
    LLM call. The invariant is broad: no live window's current file contents reach the summary input.
    """

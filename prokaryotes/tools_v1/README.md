...
- `FileTool` (`file_tool/`) — reads, creates, and edits files by line range. Read outputs are **live windows** that the harness refreshes per turn via the module's `reconcile_tracked_files()` helper, which operates on a flat `list[TurnItem]` from the active turn. Takes a `view_provider` callable (yielding the unified `list[TurnItem]`) and an optional `workspace_root` (defaults to `Path.cwd()`). See [project/features/file_tool/README.md](../../project/features/file_tool/README.md).

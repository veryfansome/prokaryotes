...
- `prokaryotes/web_v1/` — `WebBase`: FastAPI base for the chat-over-HTTP harness. Owns auth, sessions, partition reconciliation, streaming, and the compaction lifecycle. See [web_v1/README.md](web_v1/README.md).
...
- `prokaryotes/harness_v1/` — Provider-agnostic agent-loop harnesses: `ScriptHarness` (non-interactive one-off tasks), `WebHarness(WebBase)` (chat-over-HTTP), `EvalHarness` (orchestrates `ScriptHarness` runs over `EvalTask` fixtures). `WebHarness` dispatches on `impl` to select the LLM client (`AnthropicClient`/`OpenAIClient`) and instruction role (`"system"` / `"developer"`). Compaction is implemented in `WebHarness` only.
...
- `prokaryotes/eval_v1/` — Eval framework data. `EvalTask` (in `models.py`) defines a task with setup files, a setup command, and a shell `check_command` that exits 0 on success. `tasks.py` holds the curated set; the orchestrator (`EvalHarness`) lives in `prokaryotes/harness_v1/eval.py` and produces an `EvalRun` with per-task pass/fail and a tier breakdown.

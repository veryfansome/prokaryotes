...
**Streaming with tool-call continuation**: `stream_turn()` buffers each provider round, dispatches tool calls when present, and feeds results back into further LLM calls until no new tool calls are produced. `max_tool_call_rounds` caps the loop. After each round, `context_pct` usage is reported and background compaction is triggered when the configured threshold is reached. When the model emits text before stopping with `tool_use`, that text is surfaced in the web harness's NDJSON stream as a transient `progress_message` event, while tool invocations are surfaced as `tool_call` events so the UI can show activity without persisting it in the conversation tree. The persisted `ContextPartition` stores only the `function_call` / `function_call_output` items and any final assistant message; subsequent provider calls are reconstructed from those persisted items rather than from transient progress narration.
